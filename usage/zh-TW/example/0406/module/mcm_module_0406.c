    ...
    struct mcm_service_session_t *this_session)
{
    ...
    // 取得資料 (SYS 模式).
    if(mcm_config_get_alone_by_info(this_session, tmp_group, tmp_member, tmp_store,
                                    MCM_DACCESS_SYS, date) < MCM_RCODE_PASS)
    {
    ...
    {
        if(mcm_config_get_alone_by_info(this_session, tmp_group, tmp_member, tmp_store,
                                        MCM_DACCESS_NEW, date) < MCM_RCODE_PASS)
        {
            DMSG("call mcm_config_get_alone_by_info(%s) fail", path1);
            goto FREE_01;
        }
    ...
    // 取得資料 (SYS 模式).
    if(mcm_config_get_alone_by_info(this_session, tmp_group, tmp_member, tmp_store,
                                    MCM_DACCESS_SYS, ip_addr) < MCM_RCODE_PASS)
    {
        DMSG("call mcm_config_get_alone_by_info(%s) fail", path1);
        goto FREE_01;
    }
    ...
    {
        if(mcm_config_get_alone_by_info(this_session, tmp_group, tmp_member, tmp_store,
                                        MCM_DACCESS_NEW, ip_addr) < MCM_RCODE_PASS)
        {
            DMSG("call mcm_config_get_alone_by_info(%s) fail", path1);
            goto FREE_01;
        }
    ...
    // 取得資料 (SYS 模式).
    if(mcm_config_get_alone_by_info(this_session, tmp_group, tmp_member, tmp_store,
                                    MCM_DACCESS_SYS, &uptime) < MCM_RCODE_PASS)
    {
        DMSG("call mcm_config_get_alone_by_info(%s) fail", path1);
        goto FREE_01;
    }
    ...
    {
        if(mcm_config_get_alone_by_info(this_session, tmp_group, tmp_member, tmp_store,
                                        MCM_DACCESS_NEW, &uptime) < MCM_RCODE_PASS)
        {
            DMSG("call mcm_config_get_alone_by_info(%s) fail", path1);
            goto FREE_01;
        }
    ...
    // 取得資料 (SYS 模式).
    if(mcm_config_get_alone_by_info(this_session, tmp_group, tmp_member, tmp_store,
                                    MCM_DACCESS_SYS, &loading) < MCM_RCODE_PASS)
    {
        DMSG("call mcm_config_get_alone_by_info(%s) fail", path1);
        goto FREE_01;
    }
    ...
    {
        if(mcm_config_get_alone_by_info(this_session, tmp_group, tmp_member, tmp_store,
                                        MCM_DACCESS_NEW, &loading) < MCM_RCODE_PASS)
        {
            DMSG("call mcm_config_get_alone_by_info(%s) fail", path1);
            goto FREE_01;
        }
    ...
    struct mcm_service_session_t *this_session)
{
    ...
    char *path1, path2[MCM_PATH_MAX_LENGTH];
    struct mcm_config_model_group_t *tmp_group;
    struct mcm_config_store_t *tmp_parent_store, *tmp_store, *each_store;
    MCM_DTYPE_EK_TD new_key, i;
    ...
    while(tmp_store != NULL)
    {
        ...
        // 刪除第一筆, 直到沒有.
        each_store = tmp_store;
        tmp_store = tmp_store->next_store;
        if(mcm_config_del_entry_by_info(this_session, tmp_group, each_store, MCM_DACCESS_SYS)
                                        < MCM_RCODE_PASS)
        {
    ...
    {
        // 取得可用的 key
        if(mcm_config_get_usable_key_by_info(this_session, tmp_group, tmp_parent_store, &new_key)
                                             < MCM_RCODE_PASS)
        {
        ...
        if(new_key == 0)
        {
            DMSG("[usable-key] %s = entry is full", path2);
            goto FREE_01;
        }
        else
        {
            DMSG("[usable-key] %s = " MCM_DTYPE_EK_PF, path2, new_key);
        }
